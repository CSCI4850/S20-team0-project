...
#import normalize_data_utils as norm
import numpy as np
...
def scale_cutoff(value, scale_value):
    if value > scale_value:
        return 1.0
    else:
        return (value * (1.0/scale_value))


def scale_slice_with_cutoff(brain_slice, scale_factor):
    '''
    Scales each pixel by (1.0 / given value)
    if pixel value is greater than given value then pixel is set to 1.0
    '''
    normalized_slice = brain_slice.copy()
    for i in range(normalized_slice.shape[0]):
        for j in range(normalized_slice.shape[1]):
            normalized_slice[i,j] = scale_cutoff(normalized_slice[i,j], scale_factor)
    return normalized_slice


def get_modality_scale_values(brain_volume_data):
    ...
    for mod in modalities:
        all_slice_values = []
        for slice_group in brain_volume_data:
            arr = slice_group[mod].flatten()
            all_slice_values = all_slice_values + arr[ arr != 0 ].tolist()
        all_slice_values.sort()
        scale_ind = int(len(all_slice_values) * 0.95)
        scale_values[mod] = all_slice_values[scale_ind]
    return scale_values


def scale_volume_95(brain_volume_data):
    '''
    This function scales every pixel value by desired factor
    '''
    modality_scales = get_modality_scale_values(brain_volume_data)

    normalized_data = []
    # iterate through each group of slices, scale pixel values
    for slice_group in brain_volume_data:
        v_slice = {"flair": scale_slice_with_cutoff(slice_group["flair"], modality_scales["flair"]), 
                    "t1"  : scale_slice_with_cutoff(slice_group["t1"], modality_scales["t1"]), 
                    "t1ce": scale_slice_with_cutoff(slice_group["t1ce"], modality_scales["t1ce"]), 
                    "t2"  : scale_slice_with_cutoff(slice_group["t2"], modality_scales["t2"]), 
                    "seg" : scale_slice_with_cutoff(slice_group["seg"], modality_scales["seg"]),
                    "s_id": slice_group["s_id"]}
        normalized_data.append(v_slice)
    return normalized_data


def download_scaled_dataset_95():
    '''
    If scale value is None uses (1.0/slices_max_pixel_value)
    Creates a standardized dataset in dir : .../MICCAI_BraTS_2019_Data_Training/MICCAI_BraTS_2019_Data_Training/scaled_hgg
    '''
    test_path = pathlib.Path('/home/josh/Desktop/py/envs/mri-cad/MICCAI_BraTS_2019_Data_Training/MICCAI_BraTS_2019_Data_Training/test')
    #normalized_hgg    = hgg_utils.get_hgg_paths().parent.joinpath('scaled_hgg')
    normalized_hgg    = test_path
    all_patient_paths = hgg_utils.get_each_hgg_folder()
    ...
    for i in tqdm(range(len(all_patient_paths))):  
        ...
        patient_data           = hgg_utils.get_patient_data_at_index(i) # data for current patient
        normalized_data_path   = normalized_hgg.joinpath(patient_path.name)
        normalize_patient_data = scale_volume_95(patient_data)
        if not normalized_data_path.exists():
            normalized_data_path.mkdir()
